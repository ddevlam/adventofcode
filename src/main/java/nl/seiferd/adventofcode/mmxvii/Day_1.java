...
public class Day_1 {
    ...
    int partTwo(int[] input){
        final Set<Integer> checkedFrequencies = new HashSet<>(){{
            add(0); // Starting point
        }};

        int foundFrequency = checkedFrequencies.iterator().next();

        boolean searching = true;
        while(searching) {
            for (int currentFrequency : input) {
                searching = checkedFrequencies.add(foundFrequency += currentFrequency);
                if(!searching){
                    break;
                }
            }
